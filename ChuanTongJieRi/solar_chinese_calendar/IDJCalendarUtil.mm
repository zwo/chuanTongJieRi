...
{
    return ChineseCalendarDB::GetMonthDays(_year, _month);
}
...
+ (NSMutableDictionary *)jieqiWithGregYear:(NSUInteger)_year
{
    ...
    for (int i=1; i<=24; i++) {
        ...
        div_t dt=div(i, 2);        
        dc.year=_year;
        dc.month=dt.rem+dt.quot;
        dc.day=day;        
        NSDate *jieqiDate=[cal dateFromComponents:dc];        
        [dic setObject:jieqiDate forKey:[arrayJieqi objectAtIndex:i-1]];
    }
    [cal release];
    [dc release];
    return [dic autorelease];
